...
import com.doubtnutapp.databinding.ItemTrendingStaggeredListBinding
...
class TrendingStaggeredFeedViewHolder(val binding: ItemTrendingStaggeredListBinding) : BaseViewHolder<TrendingSearchDataListViewItem>(binding.root) {
    ...
    override fun bind(data: TrendingSearchDataListViewItem) {
        ...
        binding.staggeredList.adapter = childAdapter
        binding.staggeredList.layoutManager = ChipsLayoutManager.newBuilder(binding.root.context)
                .setMaxViewsInRow(3)
                .build()
        binding.staggeredList.isNestedScrollingEnabled = false
