...
import com.doubtnutapp.databinding.LayoutTrendingQuestionBinding
import com.doubtnutapp.deeplink.DeeplinkAction
import com.doubtnutapp.scheduledquiz.di.model.ScheduledQuizNotificationModel
import com.doubtnutapp.utils.UserUtil
import javax.inject.Inject

class QuizSolutionViewHolder(itemView: View) :
    ...
    val binding = LayoutTrendingQuestionBinding.bind(itemView)

    override fun bind(data: ScheduledQuizNotificationModel) {
        binding.headingSolution.text = data.heading.orEmpty()
        binding.btnSolution.text = data.btnText.orEmpty()

        binding.skipSolution.isVisible = (data.isSkippable == true)

        binding.skipSolution.setOnClickListener {
            performAction(
        ...
        binding.cvThumbnailQuizSolution.setOnClickListener {
            deeplinkAction.performAction(itemView.context, data.deeplink, Bundle().apply {
                putBoolean(Constants.CLEAR_TASK, false)
                putString(Constants.SOURCE, EventConstants.PAGE_DEEPLINK_CLICK)
            })
        ...
        binding.ivSolution.loadImageOrMathView(
            data.imageUrl.orEmpty(),
            binding.mathViewSolution,
            data.ocrText,
            placeholderId = R.drawable.placeholder_thumbnail_quiz
        )
        binding.overlaySolution.loadImageOrMathView(
            data.overlayImage.orEmpty(),
            placeholderId = R.drawable.placeholder_thumbnail_quiz
        )
        binding.headingIconSolution.loadImageOrMathView(
            data.headingIcon.orEmpty(),
            placeholderId = R.drawable.placeholder_quiz_header
        )
        binding.rootQuestion.setOnClickListener {
            performAction(
                ScheduledQuizNotificationClicked(
                    EventConstants.CLICK_QUIZ_EVENT_V2, hashMapOf(
                        EventConstants.QUIZ_CLICKED_TYPE to "new",
