...
import com.doubtnutapp.databinding.LayoutNcertVideoBinding
import com.doubtnutapp.deeplink.DeeplinkAction
import com.doubtnutapp.scheduledquiz.di.model.ScheduledQuizNotificationModel
import com.doubtnutapp.utils.UserUtil
import javax.inject.Inject

class QuizNcertViewHolder(itemView: View) :
    ...
    val binding = LayoutNcertVideoBinding.bind(itemView)

    override fun bind(data: ScheduledQuizNotificationModel) {
        binding.headingQuizNcert.text = data.heading.orEmpty()
        binding.titleQuizNcert.text = data.title.orEmpty()
        binding.subtitleQuizNcert.text = data.subTitle.orEmpty()
        binding.btnQuizNcert.text = data.btnText.orEmpty()

        binding.skipQuizNcert.isVisible = (data.isSkippable == true)

        binding.skipQuizNcert.setOnClickListener {
            performAction(
        ...
        binding.btnQuizNcert.setOnClickListener {
        ...
        binding.rootNcert.setOnClickListener {
            performAction(
                ScheduledQuizNotificationClicked(
                    EventConstants.CLICK_QUIZ_EVENT_V2, hashMapOf(
                        EventConstants.QUIZ_CLICKED_TYPE to "new",
        ...
        binding.cvThumbnailNcert.setOnClickListener {
            deeplinkAction.performAction(itemView.context, data.deeplink, Bundle().apply {
                putBoolean(Constants.CLEAR_TASK, false)
                putString(Constants.SOURCE, EventConstants.PAGE_DEEPLINK_CLICK)
            })
        ...
        binding.thumbnailQuizNcert.loadImageOrMathView(
            data.imageUrl.orEmpty(),
            binding.dmathView,
            data.ocrText,
            placeholderId = R.drawable.placeholder_thumbnail_quiz
        )
        binding.headingIconQuizNcert.loadImageOrMathView(
