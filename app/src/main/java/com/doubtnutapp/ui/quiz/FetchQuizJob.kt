...
class FetchQuizJob : DailyJob() {
    ...
    companion object {
        const val JOB_TAG = "fetch_quiz_job"

        fun enqueue(context: Context, startMs: Long? = null, endMs: Long? = null) {
            EvernoteUtils.init(context)
            WorkManager.getInstance(context).cancelUniqueWork(FetchQuizJobWorker.TAG)
            val extras = PersistableBundleCompat()
            extras.putString("authToken", authToken(DoubtnutApp.INSTANCE.applicationContext))
            scheduleAsync(
                JobRequest.Builder(JOB_TAG)
                    .setExtras(extras)
                    .setUpdateCurrent(true),
                startMs ?: TimeUnit.HOURS.toMillis(20),
                endMs ?: TimeUnit.HOURS.toMillis(21) + TimeUnit.MINUTES.toMillis(30)
            )
        }
    }
    ...
    override fun onRunDailyJob(params: Params): DailyJobResult {
        // No quiz to show for the user who hasn't logged in
            ...
            analyticsPublisher.publishEvent(
                AnalyticsEvent(
                    EventConstants.QUIZ_NOTIFICATION_TYPE, hashMapOf(
                        ...
                        EventConstants.FEATURE to Features.EVERNOTE_ANDROID_JOB
                    ), ignoreSnowplow = true
                )
            )
        ...
        // Don't show pop up if user is on Video Page.
        ...
        if (FeaturesManager.isFeatureEnabled(DoubtnutApp.INSTANCE, Features.QUIZ_POPUP)) {
            if (isLocalDataEmpty()) {
                ...
                try {
                    val quizIntent = Intent(context, ScheduledQuizNotificationActivity::class.java)
    ...
    private fun launchOldQuizActivity(): DailyJobResult {
        //If feature is disabled, launch QuizNotificationActivity
        val quizIntent = Intent(context, QuizNotificationActivity::class.java)
        quizIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        quizIntent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS)
        context.startActivity(quizIntent)
    ...
    fun launchFallbackActivity(): DailyJobResult {
        //In any fallback, launch QuizNotificationActivity
        val quizIntent = Intent(context, QuizFallbackActivity::class.java)
        quizIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        quizIntent.addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS)
        context.startActivity(quizIntent)
