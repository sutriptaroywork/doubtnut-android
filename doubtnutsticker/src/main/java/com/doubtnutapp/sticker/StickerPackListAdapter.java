...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final StickerPackListItemViewHolder viewHolder, final int index) {
        ...
        for (int i = 0; i < actualNumberOfStickersToShow; i++) {
            ...
            Glide.with(rowImage).load(
                    StickerPackLoader.getStickerAssetUri(viewHolder.imageRowView.getContext(), pack.identifier, pack.getStickers().get(i).imageFileName)
            ).into(rowImage);
            final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) rowImage.getLayoutParams();
            int marginBetweenImages = 0;
            try {
                marginBetweenImages = (viewHolder.imageRowView.getMeasuredWidth() - maxNumberOfStickersInARow * viewHolder.imageRowView.getContext().getResources().getDimensionPixelSize(R.dimen.sticker_pack_list_item_preview_image_size)) / (maxNumberOfStickersInARow - 1) - lp.leftMargin - lp.rightMargin;
