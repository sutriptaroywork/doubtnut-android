...
import static com.doubtnutapp.sticker.StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.LICENSE_AGREENMENT_WEBSITE;
import static com.doubtnutapp.sticker.StickerContentProvider.PRIVACY_POLICY_WEBSITE;
import static com.doubtnutapp.sticker.StickerContentProvider.PUBLISHER_EMAIL;
import static com.doubtnutapp.sticker.StickerContentProvider.PUBLISHER_WEBSITE;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_FILE_NAME_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_PACK_ICON_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_PACK_NAME_IN_QUERY;
import static com.doubtnutapp.sticker.StickerContentProvider.STICKER_PACK_PUBLISHER_IN_QUERY;
...
//            throw new IllegalStateException("There should be at least one sticker pack in the app");
    ...
    private static List<Sticker> getStickersForPack(Context context, StickerPack stickerPack) {
        ...
        for (Sticker sticker : stickers) {
            final byte[] bytes;
            try {
                bytes = fetchStickerAsset(context, stickerPack.identifier, sticker.imageFileName, context.getContentResolver());
                if (bytes.length <= 0) {
    ...
    public static byte[] fetchStickerAsset(Context context, @NonNull final String identifier, @NonNull final String name, ContentResolver contentResolver) throws IOException {
        try (final InputStream inputStream = contentResolver.openInputStream(getStickerAssetUri(context, identifier, name));
    ...
    public static Uri getStickerAssetUri(Context context, String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_FILE).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(context.getFilesDir().getPath()).appendPath("stickers_asset").appendPath(identifier).appendPath(stickerName).build();
    }
}
