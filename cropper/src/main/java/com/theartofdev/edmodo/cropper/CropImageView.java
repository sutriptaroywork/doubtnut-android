...
public class CropImageView extends FrameLayout {
    ...
    /**
     *  options that can be set to customize crop image
     */
    private CropImageOptions options = null;

    /**
     * The coordinates to draw rectangle
     */
    ...
    /**
     * callback to be invoked when crop overlay is initialised.
     */
    ...
    public CropImageView(Context context, AttributeSet attrs) {
        ...
        if (intent != null) {
            Bundle bundle = intent.getBundleExtra(CropImage.CROP_IMAGE_EXTRA_BUNDLE);if (bundle != null) {
                options = bundle.getParcelable(CropImage.CROP_IMAGE_EXTRA_OPTIONS);
                rectF = bundle.getParcelable(CropImage.CROP_IMAGE_EXTRA_RECTF);
            }
        }
        
        setUpCropView(context, attrs);

        if (rectF != null) {
            mCropOverlayView.setInitialAttributeValues(options, rectF);
        }
    }
    
    private void setUpCropView(Context context, AttributeSet attrs) {
        if (options == null) {
                ...
                new CropOverlayView.CropWindowChangeListener() {
                    ...
                    public void onCropWindowChanged(boolean inProgress) {
                        ...
                        if (listener != null && !inProgress) {
                            listener.onCropOverlayReleased(getCropRect(), getCropWindowRect());
                        }
                        OnSetCropOverlayMovedListener movedListener = mOnSetCropOverlayMovedListener;
                        if (movedListener != null && inProgress) {
                            movedListener.onCropOverlayMoved(getCropRect(), getCropWindowRect());
                        }
                    }
                });
        mCropOverlayView.setCropWindowInitListener(
                new CropOverlayView.CropWindowInitListener() {
                    @Override
                    public void onCropWindowInit() {
                        CropWindowInitListener listener = mCropWindowInitListener;
                        if (listener != null) {
                            listener.onCropWindowInit(getCropWindowRect());
                        }
                    }
                }
        );
    ...
    /**
     * Set the callback when the cropping is initialised
     */
    ...
    /**
     * @see CropOverlayView#setIsBorderTransparent(boolean)
     */
    public void setIsBorderTransparent(boolean value) {
        mCropOverlayView.setIsBorderTransparent(value);
    }
    ...
    public interface OnSetCropOverlayReleasedListener {
         ...
         * @param cropWidowRect The crop window's position relative to the parent's view at screen.
         */
        void onCropOverlayReleased(@Nullable Rect rect, @Nullable RectF cropWidowRect);
    }
    ...
    public interface OnSetCropOverlayMovedListener {
         ...
         *
         * @param rect The rect coordinates of the cropped overlay adjusted to the image view's dimens.
         * @param cropWidowRect The crop window's position relative to the parent's view at screen.
         */
        void onCropOverlayMoved(@Nullable Rect rect, @Nullable RectF cropWidowRect);
    }

    /**
     * Interface definition for a callback to be invoked when the crop overlay is initialised.
     */
    public interface CropWindowInitListener {

        /**
         * Called when the crop overlay is initialised
         *
         * @param cropWidowRect The crop window's position relative to the parent's view at screen.
         */
        void onCropWindowInit(@NonNull RectF cropWidowRect);
    }
