...
public class CropOverlayView extends View {
    ...
    private CropWindowInitListener mCropWindowInitListener;
    ...
    /**
     * The Paint used to draw the transparent borders around the crop area.
     */
    private Paint mBorderPaintTransparent;
    ...
    /**
     * Borders drawn after this variable is true will be transparent unless set to false again
     */
    private boolean isBorderTransparent = false;
    // endregion
    ...
    public void setCropWindowInitListener(CropWindowInitListener listener) {
        mCropWindowInitListener = listener;
    }
     ...
     */
    public void setInitialAttributeValues(CropImageOptions options, RectF rectF) {
        ...
        mInnerCirclePaint.setColor(options.borderLineColor);
        mInnerCirclePaint.setStyle(Paint.Style.FILL);

        mOuterCirclePaint = new Paint();
        mOuterCirclePaint.setColor(options.borderLineColor);
        mOuterCirclePaint.setStyle(Paint.Style.FILL);

        mBorderPaintTransparent = new Paint();
        mBorderPaintTransparent.setColor(Color.TRANSPARENT);
        mBorderPaintTransparent.setStyle(Paint.Style.FILL);
    ...
    /**
     * Borders drawn after this function is called with <code>true</code> will be transparent,
     * unless the function is called again with <code>false</code>
     * @param value the value to be set. <code>true</code> for transparent borders,
     *              <code>false</code> for normal opaque borders
     */
    public void setIsBorderTransparent(boolean value) {
        if (isBorderTransparent != value) {
            isBorderTransparent = value;
            invalidate();
        }
    }
    ...
    private void initCropWindow() {
        ...
        // Call to get initial crop rect position
        callOnCropWindowInit();
    }
...
//    drawCorners(canvas);
    ...
    private void drawBorders(Canvas canvas) {
        ...
        if (mBorderPaint != null) {
            ...
            Paint borderPaint = isBorderTransparent ? mBorderPaintTransparent : mBorderPaint;
            Paint outerCirclePaint = isBorderTransparent ? mBorderPaintTransparent : mOuterCirclePaint;
            Paint circlePaint = isBorderTransparent ? mBorderPaintTransparent : mCirclePaint;
            Paint innerCirclePaint = isBorderTransparent ? mBorderPaintTransparent : mInnerCirclePaint;
            ...
            canvas.drawCircle(rect.left, rect.top, 30, outerCirclePaint);
            canvas.drawCircle(rect.left, rect.top, 20, circlePaint);
            canvas.drawCircle(rect.left, rect.top, 10, innerCirclePaint);

            canvas.drawCircle(rect.left, rect.bottom, 30, outerCirclePaint);
            canvas.drawCircle(rect.left, rect.bottom, 20, circlePaint);
            canvas.drawCircle(rect.left, rect.bottom, 10, innerCirclePaint);

            canvas.drawCircle(rect.right, rect.top, 30, outerCirclePaint);
            canvas.drawCircle(rect.right, rect.top, 20, circlePaint);
            canvas.drawCircle(rect.right, rect.top, 10, innerCirclePaint);

            canvas.drawCircle(rect.right, rect.bottom, 30, outerCirclePaint);
            canvas.drawCircle(rect.right, rect.bottom, 20, circlePaint);
            canvas.drawCircle(rect.right, rect.bottom, 10, innerCirclePaint);
    ...
    /**
     * Invoke on crop initialised listener safe, don't let the app crash on exception.
     */
    private void callOnCropWindowInit() {
        try {
            if (mCropWindowInitListener != null) {
                mCropWindowInitListener.onCropWindowInit();
            }
        } catch (Exception e) {
            Log.e("AIC", "Exception in crop window init", e);
        }
    }
    // endregion
    ...
    /**
     * Interface definition for a callback to be invoked when crop window rectangle is initialised.
     */
    public interface CropWindowInitListener {

        /**
         * Called after crop window rectangle is initialised.
         */
        void onCropWindowInit();
    }
    // endregion
